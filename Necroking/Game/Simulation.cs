...
public class Simulation
{
    ...
    public void Tick(float dt)
    {
        ...
        foreach (var hit in _projectiles.Hits)
        {
            ...
            // Combat projectile (arrow / spell projectile / dev fireball): flag +
            // DRN derivation, knockback physics, hit dispatch, and combat logging
            // all live in Combat.AttackResolver.
            Combat.AttackResolver.ResolveProjectileImpact(this, hit);
        }
        ...
        foreach (var ld in lightningDmg)
            Combat.AttackResolver.ResolveLightningDamage(this, ld);
        PhaseEnd("lightning");
    ...
    // Attack resolution — pending-attack dispatch, the melee/sweep/arrow/spell
    // damage formulas, and the on-hit riders (knockdown, limb chop, weapon
    // bonus effects) — lives in Combat/AttackResolver.cs (Combat.AttackResolver).
    ...
    private void UpdateKnockdownRecovery(float dt)
    {
        for (int i = 0; i < _units.Count; i++)
        {
            ...
            else
            {
                ...
                _units[i].KnockdownCheckTimer = Combat.AttackResolver.KnockdownCheckInterval;
