...
- **Hit-react pass (this round)** — user-reported: fleeing units flinch when hit (should keep running) + units slide while attacking. Fixes:
  - **Flee flinch suppression** — all on-hit BlockReact application now funnels through `DamageSystem.ApplyHitReactAnim(units, idx)`, which skips the flinch when the unit is fleeing (`Fleeing` flag — set by DeerHerd RoutineFleeing + legacy FleeWhenHit — or morale `Routing` or `FleeTimer > 0`). `HitReacting` is still set unconditionally so AI flee/retarget logic is unchanged; only the *animation* is suppressed. This also closes §1.4 (the helper guards `JumpPhase != 0`) and unifies the three former direct-`SetOverride` hit sites (DamageSystem physical, melee hit, melee block).
  - **Plant while attacking** — `UpdateMovement` now hard-locks (`Velocity=0`, skip ORCA) any non-player unit that is `InCombat`, not just during `PendingAttack`/`PostAttackTimer`. Covers the pre-roll windup (which starts the swing anim before `PendingAttack` is set — see §2.5) and ORCA drift between swings, so melee units "stop to fight". Ranged units never set `EngagedTarget` so they keep their kite.
  - **Focus-fire flinch refractory** — `FlinchRefractoryTimer` (0.6s) gates re-flinch so a swarmed unit isn't perpetually staggered out of acting.
  - **Poison no longer flinches** — legacy render reads the new `HitReactTimer` (set only by `ApplyHitReactAnim`, never for poison) instead of the raw `HitReacting` flag, matching the archetype path (which already didn't flinch on poison). Deer still *flee* poison (HitReacting still set for AI).
  - Regression scenario `combat_anim_review` asserts: a sustained-fleeing DeerHerd deer takes hits without flinching and keeps running; a melee fighter's velocity stays 0 while InCombat.
- **Chase attack-anim bleed (follow-up)** — user reported a zombie deer still "attacks while moving" when chasing. Cause: a HordeMinion catches its target and swings (Engaged), the target pulls away → StandardEngagedExits flips Engaged→Chasing, which cleared `PostAttackTimer` mid-swing AND the one-shot attack OVERRIDE anim outlived the lock, so the unit slid forward (chasing) with the swing on screen. Fixes:
  - `CombatTransitions` no longer clears `PostAttackTimer` on the Engaged→Chasing / →Returning exits (still clears `PendingAttack` to avoid the teleport forever-pin). The swing now finishes with the unit planted before it chases.
  - `Game1.UpdateAnimations` (archetype path) calls new `AnimResolver.ClearOverride` to drop a stale Attack1/2/3 override once the unit is actually moving and no longer attacking (no PendingAttack, PostAttackTimer≤0, !InCombat) — the swing snaps to the run instead of bleeding. Legacy path already ties the attack anim to combat state, so no bleed there.
  - A render-driven "plant while the attack anim is on screen" flag (AttackAnimActive) was tried first but REVERTED: it stalled melee (planted units while a lingering attack anim played post-kill, before retargeting), dropping patrol_encounter to ~60%. The ClearOverride approach restored it to 6/6.
  - Regression scenario `chase_attack_anim`: a zombie deer that repeatedly engages then chases never shows an attack state in OverrideAnim while moving (asserts 0 such frames while still confirming it both attacks and chases).
